...
interface ButtonProps 
    ...
    export function ButtonHeader({title, className}: ButtonProps) {
        return(
            <button className={`btn ${className} hover:bg-gray-50 transition-all duration-300 relative overflow-hidden group`} >
                {title}
                <ChevronDown size={16} className="inline-block ml-1 transition-transform duration-200 group-hover:rotate-180"/>
